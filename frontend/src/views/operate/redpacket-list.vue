  ...
  <div>
    <Form label-position="right"
      ...
      <FormItem label="时间选择:"
        ...
        <DatePicker @on-change="pickerChange" @on-clear="clearDate" :value="selectDate" format="yyyy/MM/dd" type="date" clearable placement="bottom-end" placeholder="选择日期" style="width: 100px"></DatePicker>
        <Select v-model="selectTime" style="width:100px" clearable>
            <Option v-for="item in 24" :value="item" :key="item+'sdfhr4efdt6r'">{{(item-1)+'点-' + item +'点'  }}</Option>
        </Select>
      </FormItem>
    ...
    async queryList () {
      try {
        ...
        if (this.selectDate && !this.selectTime) {
          data.start_time = this.selectDate + ' 00:00:00'
          data.end_time = this.selectDate + ' 23:59:59'
        }
        if (this.selectDate && this.selectTime) {
          data.start_time = this.selectDate + ' ' + (this.selectTime - 1) + ':00:00'
          data.end_time = this.selectDate + ' ' + (this.selectTime < 24 ? (this.selectTime + ':00:00') : ('23:59:59'))
        }
        const result = await this.$store.dispatch('getRedPacketList', data)
