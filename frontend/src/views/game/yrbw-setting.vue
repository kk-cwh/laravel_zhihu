  ...
  <div>
    <Form label-position="right" :label-width="70" inline>
      <!-- <FormItem label="状态:" class="">
        <Select v-model="query.IsLocked" style="width:100px" clearable>
          <Option v-for="(item,key) in stateObj" :value="key" :key="key">{{ item }}</Option>
        </Select>
      </FormItem>
      <FormItem label="游戏名:" class="">
        <Input clearable type="text" v-model="query.AgentName" placeholder="填写游戏名"></Input>
      </FormItem>
      <FormItem label="游戏房间:" class="">
        <Input clearable type="text" v-model="query.Name" placeholder="填写游戏房间"></Input>
      </FormItem> -->
    ...
    <Modal v-model="showEdit" title="修改月入百万设置" width="480">
      <Form :model="editRow" ref="editFormValidate" label-position="right" :label-width="140">
        <FormItem label="pk_id" prop="pk_id">
          <Input v-model="editRow.pk_id" style="width:200px" disabled></Input>
        </FormItem>
        <FormItem label="联系号码" prop="id" :rules="{ required: true, message: '请输入！', trigger: 'blur' }">
          <Input v-model="editRow.id" style="width:200px"></Input>
        </FormItem>
        <FormItem label="联系人" prop="name" :rules="{ required: true, message: '请填写！', trigger: 'blur' }">
  ...
  methods: {
    ...
    async queryList () {
      try {
        ...
        const result = await this.$store.dispatch('getYRBWSetting', data)
    ...
    async saveEdit (data) {
      try {
        this.loading = true
        const res = await this.$store.dispatch('updateYRBWSetting', data)
