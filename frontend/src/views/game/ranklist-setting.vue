<template>
  <div>
    ...
    <Modal v-model="showEdit" title="修改排行奖励" width="580" >
      <Form :model="editRow" ref="editFormValidate" label-position="right" :label-width="180">
        ...
        <FormItem label="排行名字"   >
          <Input v-model="editRow.RankName" style="width:200px"></Input>
        </FormItem>
        <FormItem label="排行名次"   >
          <Input v-model="editRow.RankNum" style="width:200px"></Input>
        </FormItem>
        <FormItem label="排行奖励"   >
          <Input v-model="editRow.RewardNum" style="width:200px"></Input>
        </FormItem>
  ...
  methods: {
    ...
    async queryList () {
      try {
        ...
        const result = await this.$store.dispatch('getRankListSetting', data)
    ...
    async saveEdit (data) {
      try {
        this.loading = true
        const res = await this.$store.dispatch('updateRankListSetting', data)
