<template>
  <div>
    ...
    <Modal v-model="showEdit" title="修改推广设置" width="580" :styles="{top: '10px'}">
      <Form :model="editRow" ref="editFormValidate" label-position="right" :label-width="140">
        <div  v-for="(item,index) in columns" :prop="item.key" :key="index+'sdfglt'" >
        <FormItem :label="item.title"   v-if="item.key!=='action'" >
          <Input v-model="editRow[item.key]" style="width:300px"></Input>
        </FormItem>
        </div>
  ...
  methods: {
    ...
    async queryList () {
      try {
        ...
        const result = await this.$store.dispatch('getPromoSetting', data)
    ...
    async saveEdit (data) {
      try {
        this.loading = true
        const res = await this.$store.dispatch('updatePromoSetting', data)
