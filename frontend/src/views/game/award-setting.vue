<template>
  <div>
    ...
    <Modal v-model="showEdit" title="修改推广设置" width="580">
      <Form :model="editRow" ref="editFormValidate" label-position="right" :label-width="140">
        <FormItem label="奖励标准" prop="AwardLimit" :rules="{ required: true, message: '请输入奖励标准', trigger: 'blur' }">
          <Input v-model="editRow['AwardLimit']" style="width:300px"></Input>
        </FormItem>
        <FormItem label="奖励比例" prop="AwardRate" :rules="{ required: true, message: '请输入奖励比例', trigger: 'blur' }">
          <Input v-model="editRow['AwardRate']" style="width:300px"></Input>
        </FormItem>
  ...
  methods: {
    ...
    async queryList () {
      try {
        ...
        const result = await this.$store.dispatch('getAwardSetting', data)
    ...
    async saveEdit (data) {
      try {
        this.loading = true
        const res = await this.$store.dispatch('updateAwardSetting', data)
