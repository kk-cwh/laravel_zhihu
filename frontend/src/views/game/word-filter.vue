<template>
  <div>
    ...
    <Modal v-model="showEdit" title="修改过滤字符" width="580" >
      <Form :model="editRow" ref="editFormValidate" label-position="right" :label-width="180">

        <FormItem label="rowid"   >
          <Input v-model="editRow.rowid" style="width:200px" disabled></Input>
        </FormItem>
        <FormItem label="原过滤字符"  >
          <Input v-model="editRow.oldName" style="width:200px" disabled></Input>
        </FormItem>
        <FormItem label="过滤字符"   >
          <Input v-model="editRow.FilterName" style="width:200px"></Input>
        </FormItem>
  ...
  methods: {
    ...
    async queryList () {
      try {
        ...
        const result = await this.$store.dispatch('getFilter', data)
    ...
    showEditModal (index) {
      this.showEdit = true
      this.editRow = { oldName: this.tableDatas[index].FilterName,
    ...
    async saveEdit (data) {
      try {
        this.loading = true
        const res = await this.$store.dispatch('updateFilter', data)
    ...
    async add (data) {
      try {
        this.loading = true
        const res = await this.$store.dispatch('insertFilter', data)
        if (res.data && res.data.hasOwnProperty('result') && res.data.result !== 0) {
          this.$Message.error(res.data.szresult)
        } else {
          this.showAdd = false
