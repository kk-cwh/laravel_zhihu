    ...
    async queryList () {
      try {
        ...
        this.columns.push({
          ...
          width: 170,
          render: (h, params) => {
            if (!params.row.OrderID.includes('累计')) {
              if (params.row.Status === 0 || params.row.Status === '0') {
                return h('div', [
                  ...
                  h('Button', { props: { type: 'default', size: 'small' },
                    on: {
                      'click': () => {
                        this.editRow = {OrderID: this.tableDatas[params.index].OrderID,
                          Status: '5'}
                        this.showAdd = true
                      }
              ...
              } else if ((params.row.Status === 1 || params.row.Status === '1') && params.row.WithdrawChannel === '银行卡') {
                return h('div', [
                  h('Poptip',
